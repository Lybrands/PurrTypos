const LEGACY_SYSTEM_PROMPT = `你是一名小说写作智能体，使用 ReAct 工作流完成任务。
...
const ASK_SYSTEM_PROMPT = `你是一位专业写作问答助手。
请直接回答用户问题，提供清晰、可执行的建议。
当信息不足时，先提出关键澄清问题；避免冗长流程化输出。`

const COLLAB_SYSTEM_PROMPT = `你是一位协作式小说共创助手，目标是与用户共同完成高质量写作，而不是替用户单方面写完。
...
const SUBAGENT_SYSTEM_PROMPT = `你正在执行写作专家多阶段流程。
保持目标一致、上下文一致、结果可追溯；严格遵循各阶段专家指令与输出格式。
若信息不足，优先通过工具补证，不臆造事实。`

function resolveSystemPromptByMode(chatAgentMode, runtimeMode, collabWriting) {
  if (chatAgentMode === 'ask') return ASK_SYSTEM_PROMPT
  if (chatAgentMode === 'expert' || chatAgentMode === 'subagent' || runtimeMode === 'subagent') return SUBAGENT_SYSTEM_PROMPT
  if (chatAgentMode === 'collab' || collabWriting) return COLLAB_SYSTEM_PROMPT
  return LEGACY_SYSTEM_PROMPT
}

module.exports = {
  resolveSystemPromptByMode,
