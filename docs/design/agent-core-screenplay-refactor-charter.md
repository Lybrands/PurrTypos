...
1. 把已经膨胀的 Agent Core 编排实现拆成可独立验证的阶段，同时保持 Run 生命周期、原子提交、恢复、取消和工具安全语义不变；
...
| 层 | 拥有的输入/状态/结果 | 明确禁止 |
|---|---|---|
| `agent_core` | `RunRequest`、`TaskSpec`、通用 `ExecutionRecipe`、Run 状态、lease、恢复额度、工具执行证据、usage、Core Event、`RunOutcome` | Project、Stage、Operation、Candidate、Revision、剧本工具名称或 DTO |
...
routers / frontend
        ↓
application/screenplay_agent ─────→ agent_core public contracts
        ↓                                  ↑
domains/screenplay                         │
        ↑                                  │
...
拆分期间旧 `agent_core.contracts` 和 `agent_core.ports` 可以作为纯 re-export facade 暂时保留；不得在 facade 中加入行为或产品分支。
...
- 旧的 `agent_core.runtime`、`agent_core.engine`、`agent_core.contracts` 和 `agent_core.ports` 导入保持兼容；
- 保持事件、状态、错误码和持久化顺序不变；
