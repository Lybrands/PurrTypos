...
| 文件 | 准备时规模 | 主要混合职责 |
|---|---:|---|
| `backend/agent_core/runtime.py` | 3341 行 | 模型轮次、工具轮次、恢复、预算、终止与最终回答 |
| `backend/agent_core/engine.py` | 2814 行 | Context、Planner、Admission、inline/durable dispatch |
| `backend/agent_core/contracts.py` | 2083 行 | 多个独立子域的公共类型 |
...
- `agent_core/run_state.py` 与 `run_controller.py` 的状态迁移；
...
这一阶段没有改变公共入口、事件类型、错误码、Run 状态迁移或 Repository 提交顺序。`test_agent_core_phase_one_structure.py` 现在同时保护三件事：旧单文件不能恢复、已经迁走的定义不能回流、兼容 facade 与 orchestrator 只能继续缩小。
...
| 边界 | Phase 2 结果 | 权威位置 |
|---|---|---|
| Run 与宿主命令关联 | Core 仅保存不透明且不可变的 `RunBinding` | `agent_core/contracts/host.py`、`ai_agent_runs.binding_*` |
