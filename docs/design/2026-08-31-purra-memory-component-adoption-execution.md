...
这不等于真实环境已经切换完成。尚未执行真实旧数据盘点/一次性转存、收费 LLM/Embedding 中文质量验证、运行中用户服务重启和浏览器/Electron 完整应用 E2E；Planner 协议流不在该阶段范围内。现有用户服务和真实数据均未触碰。

真实数据转存、付费调用、运行中服务切换和发布保持原计划的独立边界；
