...
复用现有 `AiPanel` 的全部能力（多会话、记忆、prompt 模板、subagent、工具调用），作为主舞台。
...
### 模式（chatAgentMode）—— 全保留 5 模式

| 模式 | 现职 | 新 UI 下的角色 |
|---|---|---|
| `ask` | 纯问答 | 不出现 diff 按钮（不改正文）|
| `agent` | 单 agent + 工具 | 导演模式默认 |
| `expert` | 单专家 subagent | 高级精细控制 |
| `expert_team` | 多专家自动协作 | 重武器（如重写整章）|
| `collab` | 文字接龙共创 | **保留**，作为协作落地形态 |
...
5. ~~第 7 项 · chatAgentMode 去留~~ ✅ 全保留 5 模式
...
5. ✅ 拒绝下拉（**直接放弃** / **说明理由** — modal 输入；"再生成" 留给阶段 6 collab 链路）
