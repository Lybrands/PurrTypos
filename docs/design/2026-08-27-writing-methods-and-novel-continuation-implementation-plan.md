...
- [x] 改为受宿主约束的模型 Planner：模型按请求制定必要且非冗余的语义分析步骤，宿主不设置领域固定步骤数；校验后冻结安全 Recipe、来源范围与证据边界。
- [x] 将内置小说分析方法作为受信任上下文提供给 Planner、正文分析单元和后续追问；方法只提供可选择的分析视角和证据分层，不生成固定任务清单。
