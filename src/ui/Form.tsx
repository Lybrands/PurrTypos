...
export interface FormInstance<Values extends Record<string, any> = Record<string, any>> {
  ...
  __register: (name: string, rules?: FormRule[]) => () => void
...
const FormContext = React.createContext<FormInstance<any> | null>(null)
...
function FormBase<Values extends Record<string, any>>({
  ...
  const internalRef = React.useRef<FormInstance<Values>>()
...
function useForm<Values extends Record<string, any> = Record<string, any>>(): [FormInstance<Values>] {
  const ref = React.useRef<FormInstance<Values>>()
