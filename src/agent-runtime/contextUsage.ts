...
function latestBudgetSnapshot(messages: AgentConversationMessage[]): {
  index: number;
  message: AgentConversationMessage;
  budget: AiContextBudgetState;
  baseTokens: number;
  ...
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    ...
    return { index, message, budget, baseTokens, providerCalibrated: actualInputTokens !== null }
  }
  return null
}
...
export function calculateContextUsage(params: {
  ...
  const snapshotWindowTokens = snapshot ? nonNegativeInteger(snapshot.budget.windowTokens) : null
  ...
  const outputReserveTokens = canReuseOutputReserve
    ? Math.min(windowTokens - 1, nonNegativeInteger(snapshot?.budget.outputReserveTokens) ?? 0)
    : 0
  ...
  const snapshotBasedTokens = snapshot === null
    ? 0
    : snapshot.baseTokens + estimateConversationTokens(params.messages.slice(snapshot.index))
  const usedTokens = Math.max(visibleConversationTokens, snapshotBasedTokens)
  const providerCalibrated = snapshot?.providerCalibrated === true && selectedModelMatchesSnapshot
  return {
    usedTokens,
    windowTokens,
    inputCapacityTokens,
