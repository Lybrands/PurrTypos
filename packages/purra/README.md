...
`backend/tests/test_purra_boundaries.py` and `backend/tests/test_application_boundaries.py`
enforce this rule.
...
`backend/tests/test_purra_phase_one_structure.py` prevents the legacy files,
...
`RecipeLongTaskDispatcher` materializes that recipe as a Work Item and Long
Task, resolves each unit through a host-owned executor registry, and passes the
...
registry; neither PurrA nor the desktop UI contains writing-role names.
