...
`backend/tests/test_purra_structure.py` prevents responsibility from
flowing back into oversized facades or orchestrators.
...
```bash
python -m pytest packages/purra/tests/test_standalone_agent_conformance.py -q
python -m pytest packages/purra/tests/test_second_host_conformance.py -q
```
