...
async function main() {
  ...
  const acceptanceRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'purrtypos-writing-acceptance-'),
  )
  ...
  fs.writeFileSync(
    path.join(databaseDir, '.purrtypos-writing-replacement-acceptance'),
  ...
  console.log(`Writing acceptance workspace: ${acceptanceRoot}`)
