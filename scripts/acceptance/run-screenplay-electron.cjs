...
async function main() {
  ...
  const acceptanceRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'purrtypos-screenplay-acceptance-'),
  )
  ...
  fs.writeFileSync(
    path.join(databaseDir, '.purrtypos-screenplay-replacement-acceptance'),
  ...
  console.log(`Screenplay acceptance workspace: ${acceptanceRoot}`)
  console.log('The workspace is retained after exit for evidence inspection.')

  const child = spawn('npm', ['run', 'dev:electron:acceptance'], {
    cwd: projectRoot,
    env: {
      ...process.env,
