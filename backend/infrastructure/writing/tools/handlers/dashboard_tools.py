...
from infrastructure.writing.tools.data_loaders import (
    _list_ordered_leaf_chapters_for_book,
)
from utils.book_structure import count_words, load_chapter_texts
...
async def _tool_get_story_health_dashboard(
    ...
    db = dependencies.db
    leaves = await _list_ordered_leaf_chapters_for_book(db, str(bid))
...
async def _tool_get_writing_stats_dashboard(
    dependencies: "WritingToolDependencies",
    ctx: dict,
    args: dict,
    send_chunk: Callable | None,
