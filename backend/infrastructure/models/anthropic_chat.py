    ...
    async def _convert() -> AsyncIterator[dict]:
        ...
        try:
            ...
            if signal and signal.is_set():
                return
            raise
        # A normal iterator EOF is not an authoritative provider terminal.
        # Leave the finish reason absent so Core can classify the attempt as
        # interrupted; otherwise partial text or tool JSON could be executed
