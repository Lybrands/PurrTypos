...
async def analysis_history(db, revision_id, command_id):
    rows = await db.fetch_all(
        ...
        "AND COALESCE((SELECT session_id FROM novel_analysis_session_commands WHERE command_id=ai_agent_runs.binding_command_id), 'legacy:' || binding_aggregate_id) "
        "= COALESCE((SELECT session_id FROM novel_analysis_session_commands WHERE command_id=?), 'legacy:' || binding_aggregate_id) "
