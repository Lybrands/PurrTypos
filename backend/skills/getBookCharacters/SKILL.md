...
返回人物档案 Markdown（每人一节：姓名、人物ID、标签 + 档案全文）。未传 characterIds 与 names 时返回全部；子集筛选优先 characterIds，否则可用 names。若只要 id 与姓名的 JSON 列表、不要详情，用 listBookCharacters。修改人物前必须先用本工具读取现有档案。
