"""
Unified AI provider factory — routes chat requests to the correct
adapter (OpenAI-compatible or Anthropic) based on ``api_provider``.
"""
...
async def create_chat_stream(
    ...
    if api_provider == "anthropic":
        from services.anthropic_chat import chat_stream_as_openai_format

        return await chat_stream_as_openai_format(key, messages, request_params, signal)

    from services.openai_chat import chat_stream
...
async def create_chat_no_stream(
    ...
    if api_provider == "anthropic":
        from services.anthropic_chat import chat_no_stream_as_openai_format

        return await chat_no_stream_as_openai_format(key, messages, options, signal)

    from services.openai_chat import chat_no_stream
