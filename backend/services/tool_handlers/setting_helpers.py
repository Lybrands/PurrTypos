...
The database stores setting profiles with snake_case column names while the
tool contract uses camelCase.  Keeping that translation here prevents the two
families of tool handlers from drifting apart.
"""
...
def filter_setting_rows(
    ...
    *,
    ...
    IDs intentionally take precedence when both filters are supplied, matching
    the public tool contract used by characters and world-setting entities.
    Invalid IDs are ignored instead of failing the complete read operation.
    """
    ...
    if isinstance(names, list) and names:
        needles = [str(name).strip().lower() for name in names if str(name).strip()]
        if needles:
            return [
                row
                for row in rows
                if any(needle in str(row.get("name") or "").lower() for needle in needles)
            ]
