...
def test_durable_progress_aggregates_multiple_units_per_planner_step():
    event = AgentEvent(
        type=CoreEventType.LONG_TASK_PROGRESS,
        payload={"units": [
            {"plannerStepId": "write", "status": "completed"},
            {"plannerStepId": "write", "status": "pending"},
            {"plannerStepId": "review", "status": "completed"},
        ]},
    )

    assert _durable_plan_step_statuses(event) == {
        "write": StepStatus.RUNNING,
        "review": StepStatus.DONE,
    }
    ...
    class _Admission:
        ...
        async def evaluate(self, request, planned, signal=None):
            ...
            return TaskAdmissionDecision(
                ...
                execution_recipe=ExecutionRecipe(
                    kind="fixture.agent",
                    steps=(ExecutionRecipeStep(id="writer", kind="agent"),),
                ),
            )
        ...
        async def prepare(
            ...
            **budget,
            ...
            return ConversationCompactionResult(
                ...
                diagnostics={"decisionOwner": "purra"},
            )
