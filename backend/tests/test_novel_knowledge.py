...
@pytest.mark.asyncio
async def test_actual_composed_model_input_and_receipts_with_simulated_provider(env, monkeypatch):
    db, svc, root = env
    from application.agent_composition import set_agent_composition, clear_agent_composition
    ...
    (root / '未来.md').write_text(note('FUTURE_SECRET_MARKER', purr_id='future', temporal_scope='chapter_range', valid_from_chapter='a-3'))
    (root / '画布.canvas').write_text('{"text":"CANVAS_SECRET_MARKER"}')
    calls = []
    async def runtime(_key, messages, options, _provider, signal=None):
        calls.append(messages)
        text = json.dumps(messages, ensure_ascii=False)
        if len(calls) > 1:
            assert '铜钥匙' in text
        assert 'FUTURE_SECRET_MARKER' not in text and 'CANVAS_SECRET_MARKER' not in text
        assert str(root) not in text
        async def chunks():
            if len(calls) == 1:
                yield {'choices': [{'delta': {'tool_calls': [{'index': 0, 'id': 'knowledge-call', 'type': 'function', 'function': {'name': 'searchNovelKnowledge', 'arguments': '{"query":"红门"}'}}]}, 'finish_reason': 'tool_calls'}]}
            else:
                yield {'choices': [{'delta': {'content': '红门使用铜钥匙。'}, 'finish_reason': 'stop'}]}
        return {'applied_generation_limit': options.get('max_tokens'), 'stream': chunks(), 'model': 'model'}
    monkeypatch.setattr('infrastructure.models.provider_router.create_chat_stream', runtime)
    composition = create_agent_composition(db)
    set_agent_composition(composition)
    set_db(db)
    try:
        response = await chat_stream(ChatStreamRequest(
            messages=[{'role': 'user', 'content': '根据红门资料回答钥匙材料。'}], apiKey='test', apiProvider='openai',
            options=_fixture_model_options(), enableAgentTools=True, bookId='a', chapterId='a-1',
            chatAgentMode='agent', contextWindow='200k',
        ))
        events = await _collect(response)
        assert events[-1]['runResult']['status'] == 'done', events[-5:]
        assert len(calls) >= 2
        assert any('铜钥匙' in json.dumps(call, ensure_ascii=False) for call in calls[1:])
        rows = await db.fetch_all("SELECT payload_json FROM ai_agent_run_events WHERE event_type='stream.opened'")
        receipts = [r for row in rows for r in json.loads(row['payload_json']).get('contextEvidence', []) if r['source'] == 'novel_knowledge/v1']
        assert receipts
        assert all(r['metadata']['bookId'] == 'a' and r['metadata']['scope']['chapterId'] == 'a-1' for r in receipts)
        from routers.novel_knowledge import usage
        used = await usage('a')
        assert used['data'] and used['data'][0]['revision']
    finally:
...
@pytest.mark.asyncio
async def test_tool_run_authority_legacy_time_gate_and_checkpoint_replay(env):
    db, svc, root = env
    from application.composition_factory import create_agent_composition
    ...
    from purra.contracts import RunCreateParams, ExecutionState
    ...
    from schemas.ai import ChatStreamRequest
    from tests.test_writing_retrieval import _catalog
    composition = create_agent_composition(db)
    try:
        body = ChatStreamRequest(bookId='a', chapterId='a-1', messages=[{'role': 'user', 'content': '红门'}], apiKey='test', options={'model': 'model', 'model_profile': 'deepseek:deepseek-v4-flash', 'profile_binding': 'compatible'})
        request = await composition.prepare_request(to_writing_agent_request(body, dict(body.options)))
        options = composition.bind_run_profile(request, writing_run_options(request, {}))
        run_id = await SqliteRunRepository(db).create(RunCreateParams(session_id=None, prompt='test', mode='agent', binding=options.binding))
        retriever = NovelKnowledgeRetriever(db, svc)
        hits = await retriever.retrieve(RetrievalRequest('红门', 12, run_id=run_id))
        assert hits
        with pytest.raises(RetrievalError):
            await retriever.retrieve(RetrievalRequest('红门', 12, run_id=run_id, scope={'bookId': 'b'}))
        catalog = {r.schema.name: r for r in _catalog(db, None).registrations()}
        state = ExecutionState(run_id=run_id, domain={'bookId': 'b'})
        tool = await catalog['searchNovelKnowledge'].handler(state, {'query': '红门'})
        assert tool.context_evidence and '铜钥匙' in tool.content
        metadata = json.loads(tool.content)['hits'][0]['metadata']
        read_tool = await catalog['readNovelKnowledge'].handler(state, {'documentId': metadata['documentId'], 'revision': metadata['revision']})
        assert read_tool.context_evidence and '铜钥匙' in read_tool.content
        old_read = await catalog['getBookCharacters'].handler(state, {})
        assert old_read.error_code == 'knowledge_historical_projection_unavailable'
        future_read = await catalog['getChapterContent'].handler(state, {'chapterId': 'a-3'})
        assert future_read.error_code == 'knowledge_chapter_outside_scope'
        class Memory:
            async def validate_evidence(self, receipts, **kwargs):
                pass
        validator = NovelKnowledgeEvidenceValidator(svc, Memory(), 'a')
        await validator.validate_evidence(tool.context_evidence)
        # The public receipt roundtrip used by replay preserves the source revision.
        from purra.evidence import context_evidence_receipts
        from purra.contracts import AgentMessage
        from domains.writing.context import _context_receipt_input
        messages = (AgentMessage(role='tool', content=tool.content, tool_call_id='read', attributes={'context_name': 'searchNovelKnowledge'}, host_metadata={'context_evidence_receipts': [_context_receipt_input(r) for r in tool.context_evidence]}),)
        restored = context_evidence_receipts(messages)
        assert restored
        (root / '红门.md').unlink()
        with pytest.raises(KnowledgeError):
            await validator.validate_evidence(restored)
    finally:
        await composition.shutdown()
